...
import { Upload, Zap, Film, Download, RefreshCw, Music2, Check, Library, Plus, Trash2, Clock } from "lucide-react";
...
import ContentLibrary from "@/components/contentengine/ContentLibrary";

export default function ContentEngine() {
  const [activeTab, setActiveTab] = useState("create"); // "create" | "library"
  const [step, setStep] = useState("upload"); // upload | vibe | analyzing | results
  ...
  const [selectedProject, setSelectedProject] = useState(null); // for library view
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      const data = await base44.entities.ContentProject.filter({ created_by: user.email }, "-created_date", 50);
      setProjects(data);
    } catch (e) {
      console.error(e);
    }
  ...
  const handleFileSelect = async (file) => {
    ...
    if (!validExt) { alert("Please upload an MP3 or WAV file."); return; }
    setUploading(true);
...
Make all 4 clips use different visuals, text styles, and feel unique.`,
        response_json_schema: {
          type: "object",
          properties: {
            audio_analysis: { type: "object", properties: { bpm: { type: "number" }, energy_level: { type: "string" }, key: { type: "string" }, detected_vibe: { type: "string" }, hook_start: { type: "number" }, hook_end: { type: "number" }, hook_description: { type: "string" }, selected_lyrics: { type: "string" }, tone_description: { type: "string" } } },
            clips: { type: "array", items: { type: "object", properties: { id: { type: "number" }, title: { type: "string" }, duration: { type: "number" }, visual_style: { type: "string" }, footage_tags: { type: "array", items: { type: "string" } }, text_style: { type: "string" }, text_style_label: { type: "string" }, lyric_overlay: { type: "string" }, color_grade: { type: "string" }, motion_style: { type: "string" }, hook_section: { type: "string" }, mood_score: { type: "number" }, unique_angle: { type: "string" } } } },
          },
        },
      });
      ...
      const generatedClips = result.clips || [];
      setAudioAnalysis(analysis);
      setClips(generatedClips);

      // Save to library
      const saved = await base44.entities.ContentProject.create({
        song_filename: uploadedFile?.name,
        song_file_url: uploadedFileUrl,
        vibe: selectedVibe,
      ...
      setTimeout(() => { setStep("results"); setGenerating(false); }, 600);
    } catch (e) {
      clearInterval(progressInterval);
      setGenerating(false);
      setStep("vibe");
  ...
  const handleDeleteProject = async (id) => {
    await base44.entities.ContentProject.delete(id);
    ...
    <div className="min-h-screen bg-background px-4 py-10">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          ...
          <p className="text-muted-foreground">Upload a song. Select a vibe. Get 4 ready-to-post short-form video concepts.</p>
        </motion.div>
        ...
        <div className="flex gap-1 p-1 rounded-xl bg-secondary/40 border border-border w-fit">
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === "create" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            <Plus className="h-3.5 w-3.5" /> Create
          </button>
          <button
            onClick={() => setActiveTab("library")}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === "library" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            <Library className="h-3.5 w-3.5" /> Library
            {projects.length > 0 && (
              <span className="px-1.5 py-0.5 rounded-full bg-primary/15 text-primary text-xs font-bold">{projects.length}</span>
            )}
          </button>
        </div>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                >
                  <div className="h-16 w-16 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mx-auto group-hover:bg-primary/20 transition-colors">
                    {uploading ? <div className="h-6 w-6 border-2 border-primary/30 border-t-primary rounded-full animate-spin" /> : <Upload className="h-7 w-7 text-primary" />}
                  </div>
                  <div>
                  ...
                  {!uploading && <Button variant="outline" className="pointer-events-none">Browse Files</Button>}
                </div>
                <input ref={fileRef} type="file" accept=".mp3,.wav,audio/mpeg,audio/wav" className="hidden" onChange={(e) => handleFileSelect(e.target.files[0])} />
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    { icon: Zap, color: "text-chart-4", bg: "bg-chart-4/10", title: "4 Unique Clips", desc: "Different visuals, text styles, and angles — built for vertical video" },
                  ].map((f) => (
                    <div key={f.title} className="rounded-xl bg-card border border-border p-4 space-y-2">
                      <div className={`h-8 w-8 rounded-lg ${f.bg} flex items-center justify-center`}><f.icon className={`h-4 w-4 ${f.color}`} /></div>
            ...
            {step === "vibe" && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div className="rounded-xl bg-primary/5 border border-primary/20 p-4 flex items-center gap-3">
                  <div className="h-9 w-9 rounded-lg bg-primary/10 flex items-center justify-center shrink-0"><Music2 className="h-4 w-4 text-primary" /></div>
                ...
                <Button className="w-full h-12 font-heading font-bold text-base gap-2" disabled={!selectedVibe} onClick={handleGenerate}>
                  <Zap className="h-4 w-4" /> Generate My Clips
                </Button>
              </motion.div>
            )}
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 py-8">
                <div className="text-center space-y-6">
                  <div className="relative h-20 w-20 mx-auto">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center"><Film className="h-8 w-8 text-primary" /></div>
                  </div>
                  <div>
                  ...
                  <div className="max-w-xs mx-auto space-y-2">
                    <div className="h-2 rounded-full bg-secondary overflow-hidden">
                      <motion.div className="h-full rounded-full bg-primary" animate={{ width: `${generationProgress}%` }} transition={{ duration: 0.4 }} />
                    </div>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{uploadedFile?.name} · <span className="capitalize">{selectedVibe}</span> vibe · <span className="text-primary">Saved to library</span></p>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    <Button variant="outline" size="sm" className="gap-2" onClick={() => { setSelectedVibe(null); setStep("vibe"); setClips([]); setAudioAnalysis(null); }}>
                      <Zap className="h-3.5 w-3.5" /> Change Vibe
                    </Button>
                    <Button variant="outline" size="sm" className="gap-2" onClick={() => { setStep("analyzing"); setClips([]); setAudioAnalysis(null); handleGenerate(); }}>
                      <RefreshCw className="h-3.5 w-3.5" /> Regenerate
                    </Button>
                    <Button variant="outline" size="sm" className="gap-2" onClick={handleReset}>
                      <Upload className="h-3.5 w-3.5" /> New Song
                    </Button>
                    <Button size="sm" className="gap-2" onClick={() => setActiveTab("library")}>
                      <Library className="h-3.5 w-3.5" /> View Library
                    </Button>
                  </div>
                </div>
